...
            console.log("스플래시: 건너뛰기.");
            initializePage(); // 바로 초기화
        } else {
            console.log("스플래시: 표시 시작 (2초)");
            setTimeout(() => {
                if (splashScreen) {
...
                    }, 500);
                } else { console.warn("스플래시 요소 없음"); }
                if (mainContainer) {
                    mainContainer.style.display = 'flex';
                    console.log("스플래시: 메인 컨텐츠 표시.");
                } else { console.warn("메인 컨테이너 요소 없음"); }
                try { sessionStorage.setItem('hasSplashBeenShown', 'true'); }
                catch(e) { console.error("SessionStorage 쓰기 오류:", e); }
                initializePage(); // 스플래시 후 초기화
            }, 2000);
        }
    }
...
            if (playerListDiv && allTeamsContainer && attendeeCountSpan) {
                setupDropZones();
                renderAll();
                console.log("초기화 완료");
            } else {
                console.error("초기화 실패: 필수 DOM 요소 못 찾음.");
                 if (mainContainer) mainContainer.innerHTML = "<p style='color:red;'>페이지 로딩 오류.</p>";
            }
        } catch (error) {
             console.error("초기화 중 오류:", error);
             if (mainContainer) mainContainer.innerHTML = `<p style='color:red;'>초기화 오류: ${error.message}</p>`;
        }
    }
...
        // 선수 추가 (배정 페이지에서)
        if (addPlayerButton) { addPlayerButton.addEventListener('click', () => {
            console.log("선수 추가 클릭됨!");
            const name=newPlayerNameInput.value.trim();
            const cost=parseInt(newPlayerCostSelect.value);
            const tier=getTierFromCost(cost);
            if(name===""){alert('이름을 입력해주세요.');return;}
            if(players.some(p=>p.name===name)){alert('중복된 이름입니다.');return;}
            const newPlayer={name,cost,tier};
            players.push(newPlayer);
            if(!(name in attendanceStatus)){attendanceStatus[name]=false;} // 새 선수 출석 상태 추가
            players.sort((a,b)=>a.name.localeCompare(b.name));
            saveData();
            renderAll();
            newPlayerNameInput.value='';
        }); } else { console.error("addPlayerButton 못 찾음"); }

        // ★★★ 자동 배정 (수정된 'Greedy' 로직) ★★★
        if (autoAssignButton) {
            autoAssignButton.addEventListener('click', () => {
                console.log("자동 배정 버튼 클릭 감지! (Greedy 밸런스)");
...
                // 2. 배정할 선수 목록 (참석 O, 배정 X, 안정성 검사)
                let unassignedAttendees = players
                    .filter(p => p && p.name && attendanceStatus[p.name] === true && !assignedPlayerNames.has(p.name));
                
                // --- 3단계부터 'Greedy' 로직 시작 ---

                // 3. [밸런스] 배정할 선수들을 '코스트가 높은 순'으로 정렬 (안정성 검사)
                unassignedAttendees.sort((a, b) => (b.cost || 0) - (a.cost || 0));
                ...
                for (const playerToAssign of unassignedAttendees) {
                    ...
                    if (!allTeams || allTeams.length === 0) {
                        console.warn("배정할 팀이 없습니다.");
                        break; // 팀이 하나도 없으면 중단
                    }
                    const memberCounts = allTeams.map(t => t.members.length);
                    ...
                    // 2순위: 그중에서 코스트가 가장 낮은 순으로 정렬 (안정성 검사)
                    eligibleTeams.sort((a, b) => (a.cost || 0) - (b.cost || 0));
